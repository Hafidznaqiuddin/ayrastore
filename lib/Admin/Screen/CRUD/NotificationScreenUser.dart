...
class _NotificationScreenState extends State<NotificationScreenUser> {
  late Stream<QuerySnapshot<Map<String, dynamic>>> _notificationStream;

  @override
  void initState() {
    super.initState();
    _getNotificationsForCurrentUser();
  }

  void _getNotificationsForCurrentUser() {
        ...
        child: Dismissible(
          ...
          child: ListTile(
            leading: Image.network(
              notification['images']?[0] ?? '', // Use empty string if images is null or empty
              width: 50,
              height: 50,
              fit: BoxFit.cover,
            ),
            title: Text(
              notification['brand'] ?? '', // Use empty string if brand is null
              style: TextStyle(fontWeight: FontWeight.bold),
            ),
            subtitle: Text('${notification['category'] ?? ''} - RM${notification['price'] ?? ''}'), // Use empty string if category or price is null
          ),
        ),
      ),
    );
